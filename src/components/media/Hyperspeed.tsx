...
// @ts-nocheck
"use client";
// Vendored from React Bits — Hyperspeed (three.js + postprocessing). Kept verbatim
// except: the `./Hyperspeed.css` import is dropped (styles are scoped in
// globals.css under #lights), and the renderer pixel ratio is capped at 1.5 so
// the effect doesn't tax scroll on hi-dpr displays.
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    // Guard the async init: if the effect cleaned up (dispose → forceContextLoss)
    // while assets were still loading, running init() on the dead context throws
